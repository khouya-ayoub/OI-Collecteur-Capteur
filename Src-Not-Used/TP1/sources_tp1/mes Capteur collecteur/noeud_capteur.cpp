...
std::string ip_addr_df("127.0.1.1");
int numeroData = 1;
...
void* notifyDataAvailable(void *i) {
	...
	sock_send(clt_sock, EVT_DATA_AVAILABLE);
...
void* clientFunc(void *n) {
	while(1) {
		...
		//usleep(CHECK_INTERVAL);
			...
			FILE* datafile;
			std::string filename(dataDirectory + "/" + ip_addr_df + "_data_" + to_string(numeroData));
			datafile = fopen(filename.c_str(), "w");
			fclose(datafile);
			
			dataListMutex.lock();
			std::cout << "[CLIENT] Adding Data" << filename.c_str() << std::endl;
			dataList.push_back(ip_addr_df + "_data_" + to_string(numeroData++));
			dataListMutex.unlock();
